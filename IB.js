...
function finaliserCreation() {
    ...
    // Récupération des paramètres d'agenda
    ...
    // Récupération du tel et email pour la mémoire intelligente
    const telBoss = document.getElementById('creer-telephone').value.trim();
    const emailBoss = document.getElementById('creer-email').value.trim();

    if(!asso || !boss || !dateDeb) {
        afficherModale("Oups !", "Veuillez remplir tous les champs (y compris la date) pour créer votre tontine.", "⚠️");
        return;
    }
    ...
    // [INTELLIGENCE] Sauvegarde immédiate en session pour la fiche personnelle future
    localStorage.setItem('tontine_current_user_phone', telBoss);
    localStorage.setItem('tontine_current_user_email', emailBoss);
    ...
    // Entre dans l'application avec la session du Président
